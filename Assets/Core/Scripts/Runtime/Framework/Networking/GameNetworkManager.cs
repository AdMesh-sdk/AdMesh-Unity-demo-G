...
namespace Blocks.Gameplay.Core
{
    ...
    public class GameNetworkManager : NetworkManager
    {
        ...
        {
            if (IsListening || IsServer || IsClient || ShutdownInProgress)
            {
                Debug.LogWarning("[GameNetworkManager] Ignoring StartHostConnection because the network stack is already active or shutting down.", this);
                return;
            }
        ...
        public void StartClientConnection()
        {
            if (IsListening || IsServer || IsClient || ShutdownInProgress)
            {
                Debug.LogWarning("[GameNetworkManager] Ignoring StartClientConnection because the network stack is already active or shutting down.", this);
                return;
            }
